...
SYSTEM_TEMPLATES = [
    ...
    },
    {
        ...
        "content": (
            ...
            "и больше вопросов в expert_questions, но каждый вопрос — про конкретный пробел или неточность в документах.\n"
            ...
            "- expert_questions «Вопросы экспертам»: вопросы ТОЛЬКО там, где в исходных материалах "
            "(brief, выжимки файлов, заметки) есть пробелы, отсутствие данных, неточности, противоречия "
            "или неоднозначности. Не задавай общих вопросов «на всякий случай» — каждый вопрос должен явно "
            "опираться на конкретный фрагмент или отсутствие информации в документах. "
            "Задавай вопрос, только если ответ важен для конкретного вида работ: уточнить границы процедуры, "
            "обязательные шаги, критичные наблюдаемые нарушения, спорные нормы, граничные случаи. "
            "5–12 вопросов (больше — если много пробелов в данных). "
            "title — формулировка вопроса; description — что уже известно из материалов и какой именно пробел "
            "или неточность обнаружена (укажи источник: «в СОП … не указано …», «в brief противоречие …»); "
            "why_needed — что именно восполнит ответ для work_storylines и errors[] этого вида работ; "
...
OPERATOR_PRESETS = [
    ...
    },
    {
        ...
        "content": (
            ...
            "3. Вопросы экспертам: только там, где в материалах проекта (brief, выжимки, заметки) есть пробелы, "
            "неточности или противоречия — и ответ важен для конкретного вида работ. "
            "В description укажи, какой именно пробел в документах обнаружен. "
            "title, description, why_needed, answer (пока пустой).\n\n"
            "Не выдумывай факты. Если данных мало — фиксируй в expert_questions конкретные пробелы из материалов."
        ),
    },
    {
        "step_type": "scenario_plan",
